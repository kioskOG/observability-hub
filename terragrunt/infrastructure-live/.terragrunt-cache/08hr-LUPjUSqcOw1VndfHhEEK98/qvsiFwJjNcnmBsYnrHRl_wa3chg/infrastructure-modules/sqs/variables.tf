...
variable "oldest_message_age_threshold" {
  description = "Maximum age (in seconds) of the oldest message in the queue before triggering an alarm"
  type        = number
  default     = 3600
}

variable "dlq_messages_threshold" {
  description = "Number of messages in the Dead Letter Queue that triggers an alarm"
  type        = number
  default     = 1
}

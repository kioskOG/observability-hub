...
variable "client_id" {
  type        = string
  description = "The unique Client ID (SAML SP Entity ID or OIDC client_id)"
}
...
variable "advanced_saml_mappers" {
  type = map(object({
    mapper_type     = optional(string) # "user_attribute", "user_property", "group", "role", "hardcoded_attribute"
    protocol_mapper = optional(string) # Keycloak protocol mapper provider id (fully extensible path)
  ...
  description = "DEPRECATED: use saml_mappers. Merged with saml_mappers for backward compatibility."
  default     = {}
}

variable "saml_mappers" {
  type = map(object({
    # Fully extensible: set protocol_mapper + config for any Keycloak SAML mapper.
    protocol_mapper = optional(string)
    config          = optional(map(string), {})
    # Typed shortcuts (ignored when protocol_mapper is set):
    mapper_type     = optional(string) # user_attribute | user_property | group | role | hardcoded_attribute
    user_attribute  = optional(string)
    saml_attribute  = optional(string)
    user_property   = optional(string)
  ...
  description = "Extensible SAML protocol mappers (protocol-named; replaces advanced_saml_mappers)"
  default     = {}
}

variable "roles_mapper_name" {
  type        = string
  description = "Name of the built-in SAML role-list protocol mapper (keep default for Wazuh zero-drift)"
  default     = "wazuhRoleKey"
}

# -----------------------------------------------------------------------------
# Protocol selection + OIDC client settings
# -----------------------------------------------------------------------------

variable "client_protocol" {
  type        = string
  description = "Keycloak client protocol: saml (default) or openid-connect"
  default     = "saml"

  validation {
    condition     = contains(["saml", "openid-connect"], var.client_protocol)
    error_message = "client_protocol must be \"saml\" or \"openid-connect\"."
  }
}

variable "client_description" {
  type        = string
  description = "Optional description shown in the Keycloak admin UI"
  default     = null
}

variable "access_type" {
  type        = string
  description = "OIDC access type: CONFIDENTIAL, PUBLIC, or BEARER-ONLY"
  default     = "CONFIDENTIAL"

  validation {
    condition     = contains(["CONFIDENTIAL", "PUBLIC", "BEARER-ONLY"], var.access_type)
    error_message = "access_type must be CONFIDENTIAL, PUBLIC, or BEARER-ONLY."
  }
}

variable "client_secret" {
  type        = string
  description = "Optional OIDC client secret. Leave null for Keycloak to generate one (confidential clients)."
  default     = null
  sensitive   = true
}
...
variable "standard_flow_enabled" {
  type        = bool
  description = "Enable OAuth2 Authorization Code (standard) flow"
  default     = true
}

variable "implicit_flow_enabled" {
  type        = bool
  description = "Enable OAuth2 Implicit flow (generally leave false)"
  default     = false
}
...
variable "service_accounts_enabled" {
  type        = bool
  description = "Enable Client Credentials / service account"
  default     = false
}

variable "web_origins" {
  type        = list(string)
  description = "Allowed CORS web origins for the OIDC client"
  default     = ["+"]
}

variable "admin_url" {
  type        = string
  description = "Admin URL for the client application"
  default     = ""
}

variable "valid_post_logout_redirect_uris" {
  type        = list(string)
  description = "Valid post-logout redirect URIs (OIDC)"
  default     = []
}

variable "pkce_code_challenge_method" {
  type        = string
  description = "PKCE method: S256, plain, or empty string to disable"
  default     = "S256"
}

variable "backchannel_logout_url" {
  type        = string
  description = "OIDC backchannel logout URL"
  default     = ""
}

variable "backchannel_logout_session_required" {
  type        = bool
  description = "Whether backchannel logout requires a session"
  default     = true
}

variable "backchannel_logout_revoke_offline_sessions" {
  type        = bool
  description = "Revoke offline sessions on backchannel logout"
  default     = false
}

variable "default_scopes" {
  type        = list(string)
  description = "Default Keycloak client scopes for OIDC (do not include openid — it is not a client scope)"
  default     = ["profile", "email", "roles"]
}

variable "optional_scopes" {
  type        = list(string)
  description = "Optional client scopes for OIDC clients (e.g. offline_access)"
  default     = ["offline_access"]
}

variable "oidc_mappers" {
  type = map(object({
    # Fully extensible: set protocol_mapper + config for any Keycloak OIDC mapper.
    protocol_mapper = optional(string)
    config          = optional(map(string), {})
    # Typed shortcuts (ignored when protocol_mapper is set):
    mapper_type                 = optional(string) # user_attribute | user_property | group_membership | audience | hardcoded_claim | realm_role | client_role
    user_attribute              = optional(string)
    user_property               = optional(string)
  ...
  description = "Extensible OIDC protocol mappers. Pass app-specific claims (e.g. Grafana groups) from Terragrunt — nothing Grafana-specific is hardcoded."
  default     = {}
}
